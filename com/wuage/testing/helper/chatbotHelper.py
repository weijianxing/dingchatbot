...
class MarkDownMessage():
    ...
    def __init__(self,title):
        self.title = title
    def getBoldText(self,text):
        return "**" + text + "**"

    def getItalicText(self,text):
        return "*" + text + "*"

    def getLinkText(self,text, href):
        return "[" + text + "](" + href + ")"

    def getImageText(self,imageUrl):
        return "![image](" + imageUrl + ")"

    def getHeaderText(self,type=1, text=""):
        headerText=""
        for num in range(0,type):
            headerText+="#"
        return headerText+' ' + text
    def seTitle(self , title):
        self.title = title
    def getReferenceText(self,text):
        return "> " + text
    def add(self,item):
...
class DingtalkChatbot(object):
    ...
    def send_monitorMsg(self , title ,subTitle,owner, at_mobiles=[], reporturl="https://www.wuage.com" , is_at_all=False):
        """
        通用监控推送信息接口

        :param title: 消息主标题 如果 iOS 自动化巡检
        :param subTitle: 消息副标题 如 ：登录验证场景
        :param owner: ： 测试脚本维护人员
        :param at_mobiles: 消息@信息
        :param reporturl: 测试报告地址
        :param is_at_all: 是否@ 所有人
        :return:
        """
        mk = MarkDownMessage(title=title)
        mk.seTitle(mk.getHeaderText(2, title))
        mk.add(mk.getHeaderText(2, title)+"\n\n")
        mk.add(mk.getItalicText(subTitle)+ "\n\n")
        ownerInfo = mk.getBoldText("用例负责人：   ")+ owner + "\n\n"
        mk.add(ownerInfo)
        d = datetime.now()
        exectime = mk.getBoldText("用例执行时间： ") + str(d)[:-7] + "\n\n"
        mk.add(exectime)
        mk.add(mk.getLinkText("报告查看" , reporturl))
        data = mk.getMessage()
        data["at"] = {}
        if at_mobiles:
        ...
        print(data)
        # logging.debug('消息为： ：%s' % data)
        return self.post(data)

    def send_failInfo(self, title ,subTitle,owner, at_mobiles=[], reporturl="https://www.wuage.com" , is_at_all=False , caseInfo="" ):
        """
        当测试用例执行失败后添加 @xxx 并提示失败
        :param title:
        :param subTitle:
        :param owner:
        :param at_mobiles:
        :param reporturl:
        :param is_at_all:
        :param caseInfo: 失败描述信息
        :return:
        """
        self.send_monitorMsg(title, subTitle, owner , at_mobiles , reporturl , is_at_all )
        failInfo = caseInfo + " 执行失败"
        self.send_text(failInfo, False , at_mobiles)
...
if __name__ == '__main__':
    WEBHOOK_TOKEN = "https://oapi.dingtalk.com/robot/send?access_token=xxxxxxxx"
    ding = DingtalkChatbot(webhook=WEBHOOK_TOKEN)
    title = "iOS测试ding python sdk"
    at_mobiles= ["13520309744","魏建星"]
    # at_mobiles = ["13520309744"]
